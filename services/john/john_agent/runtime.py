    ...
    async def _chat(
        ...
        async with self._session_lock:
            ...
            if not requested_session or session_key not in self._sessions:
                # Identity (role/company) is bound to the session at CREATION and
                # is never changed on reuse. A replayed session_id therefore
                # cannot escalate its role -- the state set here is fixed for the
                # life of the session.
